...
import { FixedContainer, SlotContainer } from './styles'

class OverlayMenu extends PureComponent {
  margeStyles = memoizeOne((containerSizeStyles, wrapperStyles) => ({
    ...containerSizeStyles,
    ...wrapperStyles,
  }))
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (containerSizeStyles !== prevState.containerSizeStyles) {
      ...
      const {
        ...
        bottom: zoneBot,
        right: zoneRight,
      } = refContainer.getBoundingClientRect()
      // ширина контекстного меню
      const { clientWidth: contextMenuWidth, clientHeight: contextMenuHeight } =
        refOverlayContainer.current
      // влазит ли снизу
      ...
      const leftPosition =
        clientWidth - (left - zoneLeft) > contextMenuWidth ? left : zoneLeft
      ...
      this.setState({
        positionStyles:
          ...
          (refContainer !== document.body &&
            (outOfXAxis < 0 || zoneLeft > leftPosition))
            ? {
                wrapperStyles: {
                  ...(sticky
                    ? { boxShadow: 'rgba(0, 0, 0, 0.125) 1px 6px 11px' }
                    : {}),
                  ...YPositions,
                  ...(reversDefault || outOfXAxis < 0 // отрисовка справа на лево
                    ? {
                        ...
                        ...(zoneRight - rightPosition - contextMenuWidth < 0
                          ? { width: 'auto', left: 0 }
                          : {}),
                      }
                ...
                tipStyles: {
                  ...
                  ...(outOfYAxis < 0
                    ? {
                        transform:
                          'rotate(45deg) translateX(50%) translateZ(-1px)',
                      }
                    : {
                        transform:
                          'rotate(45deg) translateX(-50%) translateZ(-1px)',
                  ...
                  ...(reversDefault || outOfXAxis < 0
                    ...
                    : {
                        left: `${
                          leftPosition - 7 < pageX
                            ? maxTipLeftPosition > pageX
                              ? pageX - 7 > leftPosition
                                ? pageX
                                : pageX + 7
                              : maxTipLeftPosition
                            : pageX - 7
      ...
      this.observer.observe(refTargetParent, {})
      prevProps.refContainer.removeEventListener(
        'scroll',
        this.resolveDimensions,
      )
      refContainer.addEventListener('scroll', this.resolveDimensions)
  ...
  resolveDimensions = () => {
    const {
      ...
      props: {
        ...
        minSize,
        event: { target },
        refTargetParent: { clientWidth } = target,
      },
    } = this
    this.setState({
      containerSizeStyles: {
        ...
        minWidth: isNaN(Number(minSize))
          ...
          : `${minSize}px`,
        width:
          refOverlayContainer.clientWidth > clientWidth
            ? 'auto'
            : `${clientWidth}px`,
      },
    })
  ...
  render() {
    const {
      ...
      state: {
        containerSizeStyles,
        positionStyles: { wrapperStyles, containerStyles },
      },
      props: {
        children,
        positionStatic,
